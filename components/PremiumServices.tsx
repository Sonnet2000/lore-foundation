        ...
        <div className="mt-14 grid gap-6 sm:mt-16 sm:grid-cols-2 lg:grid-cols-3">
          {services.map((s) => {
            ...
            return (
              <div
                key={s.id}
                className={`card-lift group relative flex flex-col overflow-hidden rounded-3xl border transition-all ${
                  s.is_featured
                    ? "border-lore-gold/30 bg-gradient-to-b from-lore-gold/10 to-transparent dark:from-lore-gold/15"
                    : "border-lore-dark/5 bg-lore-cream dark:border-white/5 dark:bg-lore-night-card"
                }`}
              >
                {s.image_url ? (
                  <div className="relative aspect-[16/10] w-full overflow-hidden bg-lore-ink/5 dark:bg-white/5">
                    <Image
                      src={s.image_url}
                      alt={s.title}
                      fill
                    ...
                    <div className="absolute left-3 top-3 flex h-10 w-10 items-center justify-center rounded-xl bg-white/95 text-lore-gold-dark shadow-sm backdrop-blur-sm dark:bg-lore-night/90 dark:text-lore-gold-light">
                      <Icon className="h-5 w-5" />
                    </div>
                    {s.is_featured && (
                      <span className="absolute right-3 top-3 inline-flex items-center gap-1 rounded-full bg-lore-gold px-3 py-1 text-[10px] font-bold uppercase tracking-wide text-lore-dark shadow-sm">
                  ...
                  s.is_featured && (
                    <span className="absolute right-5 top-5 inline-flex items-center gap-1 rounded-full bg-lore-gold px-3 py-1 text-[10px] font-bold uppercase tracking-wide text-lore-dark">
                      <Star className="h-2.5 w-2.5" />Populaire
                    </span>
                  )
                )}
