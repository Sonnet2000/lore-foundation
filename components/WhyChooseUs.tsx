  ...
  return (
    <section className="relative px-5 py-20 sm:px-8 sm:py-28 lg:px-12 bg-lore-cream dark:bg-lore-night overflow-hidden">
      ...
      <div className="mx-auto max-w-7xl relative">
        ...
        <div className="mt-10 grid grid-cols-2 gap-4 sm:mt-18 sm:gap-7 lg:grid-cols-3 lg:gap-9">
          {whyChooseUs.map((card, i) => (
            <AnimatedSection key={card.title} delay={i * 0.12}>
              <TabCard compact className="h-full card-lift">
                {/* Icon with ring */}
                <span className="mb-3 inline-flex h-11 w-11 items-center justify-center rounded-2xl bg-lore-emerald/8 text-lore-emerald ring-1 ring-lore-emerald/15 dark:bg-lore-emerald/10 dark:text-lore-emerald-light dark:ring-lore-emerald/20 sm:mb-5 sm:h-14 sm:w-14">
                  <card.icon className="h-5 w-5 sm:h-7 sm:w-7" strokeWidth={1.75} />
                </span>

                {/* Number accent */}
                <span className="mb-1.5 block font-display text-[10px] font-bold uppercase tracking-[0.2em] text-lore-gold/70 sm:mb-3 sm:text-xs sm:tracking-[0.25em]">
                  0{i + 1}
                </span>

                <h3 className="font-display text-sm font-bold leading-snug text-lore-ink dark:text-white sm:text-xl">
                  {card.title}
                </h3>
                <p className="mt-2 line-clamp-3 text-xs leading-relaxed text-lore-ink/55 dark:text-white/55 sm:mt-3 sm:line-clamp-none sm:text-sm">
                  {card.description}
                </p>
                ...
                <button
                  ...
                  className="focus-ring group mt-4 inline-flex items-center gap-1.5 rounded text-xs font-semibold text-lore-emerald transition-colors hover:text-lore-dark dark:text-lore-emerald-light dark:hover:text-white sm:mt-6 sm:text-sm"
                >
                  Découvrir
