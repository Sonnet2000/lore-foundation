      ...
      <div className="mx-auto max-w-2xl px-5 py-12">
        {!settings.binanceEnabled ? (
          ...
          <div className="flex flex-col gap-8">
            <div>
              <h1 className="font-display text-2xl font-bold text-white sm:text-3xl">Payer avec Binance</h1>
              <p className="mt-2 text-sm text-white/60">
                Envoyez le montant convenu via Binance Pay, puis remplissez le formulaire ci-dessous pour
                que nous puissions confirmer votre paiement.
              </p>
            </div>

            {/* Instructions & infos Binance */}
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <input value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="Téléphone (optionnel)"
                  className="rounded-xl border border-white/10 bg-white/5 px-3.5 py-2.5 text-sm text-white outline-none focus:border-lore-gold" />
                <input value={amount} onChange={(e) => setAmount(e.target.value)} placeholder="Montant envoyé (ex: 50 USDT)"
                  className="rounded-xl border border-white/10 bg-white/5 px-3.5 py-2.5 text-sm text-white outline-none focus:border-lore-gold" />
              </div>
              <input value={subject} onChange={(e) => setSubject(e.target.value)} placeholder="Pour quel service ? (ex: Développement site web)"
                className="rounded-xl border border-white/10 bg-white/5 px-3.5 py-2.5 text-sm text-white outline-none focus:border-lore-gold" />
              <input value={reference} onChange={(e) => setReference(e.target.value)} placeholder="Référence / ID de transaction Binance"
                className="rounded-xl border border-white/10 bg-white/5 px-3.5 py-2.5 text-sm text-white outline-none focus:border-lore-gold" />
