...
export default function AnnouncementsPanel() {
  ...
  async function handleSave() {
    ...
    const res = await fetch(
      isNew ? "/api/admin/announcements" : `/api/admin/announcements/${editingId}`,
      {
  ...
  async function handleNotify(id: string) {
    if (!confirm("Envoyer un email à tous les abonnés pour cette annonce ?")) return;
    setNotifyingId(id);
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <p className="text-sm text-lore-ink/50 dark:text-white/50">
        L&apos;annonce active la plus récente s&apos;affiche dans une bannière en haut du
        site, juste au-dessus du menu. Une seule annonce active s&apos;affiche à la fois.
      </p>
      ...
      {editingId !== null && (
        <div className="rounded-3xl border border-lore-dark/5 bg-lore-cream/60 p-5 dark:border-white/5 dark:bg-white/[0.03]">
          ...
          <TextArea
            ...
            placeholder="🎉 Inscriptions ouvertes pour notre prochain séminaire !"
          />
          ...
          <div key={item.id} className="flex flex-col gap-1.5">
            <RowCard
              ...
              subtitle={
                item.is_active
                  ? item.notified_at
                    ? `Active · Notifiée le ${new Date(item.notified_at).toLocaleDateString("fr-FR")}`
                    : "Active"
                  : "Inactive"
              }
              thumbnail={
                <button
                  ...
                  className={`flex h-12 w-12 shrink-0 items-center justify-center rounded-xl transition-colors ${
                    item.is_active
                      ...
                      : "bg-lore-dark/5 text-lore-ink/30 dark:bg-white/5 dark:text-white/30"
                  }`}
                >
                  {item.is_active ? (
            ...
            {notifyResult?.id === item.id && (
              <p className="pl-2 text-xs font-medium text-lore-ink/60 dark:text-white/60">
                {notifyResult.text}
              </p>
            )}
          </div>
