...
type FormState = {
  title: string;
  description: string;
  starts_at: string; // datetime-local value, e.g. "2026-07-12T14:00"
  location: string;
...
export default function SeminarsPanel() {
  ...
  async function handleDelete(id: string) {
    if (!confirm("Supprimer ce séminaire ? Les inscriptions associées seront aussi supprimées.")) return;
    await fetch(`/api/admin/seminars/${id}`, { method: "DELETE" });
    if (registrationsFor === id) setRegistrationsFor(null);
    refresh();
  }

  async function toggleRegistrations(id: string) {
    if (registrationsFor === id) {
      setRegistrationsFor(null);
      return;
    }
    setRegistrationsFor(id);
  ...
  async function handleNotify(id: string) {
    if (!confirm("Envoyer un email à tous les abonnés pour ce séminaire ?")) return;
    setNotifyingId(id);
        ...
        <div className="rounded-3xl border border-lore-dark/5 bg-lore-cream/60 p-5 dark:border-white/5 dark:bg-white/[0.03]">
          <div className="grid gap-4 sm:grid-cols-2">
            <div>
              ...
              <TextInput
                ...
                placeholder="Séminaire : Réussir sa transition digitale"
              />
            </div>
            <div>
          ...
          <div className="mt-4">
            ...
            <TextInput
              ...
              placeholder="Cap-Haïtien, Nord, Haïti — ou un lien Zoom/Meet"
            />
          </div>
          ...
          <div className="mt-4">
            ...
            <TextArea
              ...
              placeholder="Présentez le séminaire : objectifs, public visé, ce que les participants vont apprendre..."
            />
          </div>

          <div className="mt-4">
            <MediaListField
              label="Photos / vidéos du séminaire"
              values={form.media}
      ...
      <div className="flex flex-col gap-2">
        {items.map((item) => (
          <div key={item.id} className="flex flex-col gap-2">
            <RowCard
              ...
              }${item.registration_open ? "" : " · Inscriptions fermées"}${
                item.notified_at
                  ? ` · Notifié le ${new Date(item.notified_at).toLocaleDateString("fr-FR")}`
                  : ""
              }`}
              thumbnail={
            ...
            {notifyResult?.id === item.id && (
              <p className="ml-[60px] text-xs font-medium text-lore-ink/60 dark:text-white/60">
                {notifyResult.text}
              </p>
            )}
            ...
            >
              ...
              <ChevronDown
                className={`h-3.5 w-3.5 transition-transform ${
                  registrationsFor === item.id ? "rotate-180" : ""
                }`}
              />
            </button>
            ...
            {registrationsFor === item.id && (
              <div className="ml-[60px] rounded-2xl border border-lore-dark/5 bg-white p-4 dark:border-white/5 dark:bg-lore-night-surface">
                {registrations === null ? (
                  ...
                  <div className="flex flex-col gap-2">
                    <p className="text-xs font-semibold text-lore-ink/50 dark:text-white/50">
                      {registrations.length} inscrit(e)s
                    </p>
                    {registrations.map((r) => (
                      <div
                        key={r.id}
                        className="flex flex-col gap-0.5 border-b border-lore-dark/5 pb-2 text-sm last:border-0 dark:border-white/5"
                      >
                        <span className="font-semibold text-lore-ink dark:text-white">
                          {r.name}
                        </span>
                        <span className="text-lore-ink/60 dark:text-white/60">
                          {r.email}
                          {r.phone ? ` · ${r.phone}` : ""}
                        </span>
                      </div>
        ...
        {items.length === 0 && (
          <p className="py-8 text-center text-sm text-lore-ink/40 dark:text-white/40">
            Aucun séminaire pour le moment.
          </p>
        )}
      </div>
    </div>
