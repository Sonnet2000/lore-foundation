...
export async function POST(request: Request, { params }: { params: { id: string } }) {
  ...
  const full_name = typeof body.full_name === "string" ? body.full_name.trim().slice(0, 150) : "";
  const phone = typeof body.phone === "string" ? body.phone.trim().slice(0, 40) : "";
  const address = typeof body.address === "string" ? body.address.trim().slice(0, 300) : "";
  const birth_date = typeof body.birth_date === "string" ? body.birth_date.trim().slice(0, 20) : "";
  const id_document_url = typeof body.id_document_url === "string" ? body.id_document_url.trim().slice(0, 500) : "";

  // Vrè fòmilè enskripsyon: non ak telefòn obligatwa, menmsi moun lan gen kont deja.
  if (!full_name || !phone) {
    return NextResponse.json({ error: "Non konplè ak nimewo telefòn obligatwa." }, { status: 400 });
  }
  ...
  try {
    const enrollment = await requestEnrollment(
      id,
      user.id,
      method || reference || proof_url ? { method: method || "moncash", reference, proof_url } : undefined,
      { full_name, phone, address, birth_date, id_document_url }
    );

    // Notifikasyon imèl (pa bloke repons lan si sa echwe — pi enpòtan enskripsyon an rive nan baz done a).
    void sendEnrollmentEmails({ course, enrollment, studentEmail: user.email ?? "", studentName: full_name });
...
async function sendEnrollmentEmails({
  ...
  course: { title: string };
  enrollment: { status: string; payment_reference: string | null; payment_proof_url: string | null; phone: string };
  studentEmail: string;
  studentName: string;
}) {
  try {
    ...
    if (contact.email) {
      await sendBulkNotification({
        ...
        html: emailLayout({
          ...
          body: `${studentName || "Yon elèv"} (${studentEmail || "imèl pa disponib"}${enrollment.phone ? ` · ${enrollment.phone}` : ""}) mande enskri nan <strong>${course.title}</strong>.${
